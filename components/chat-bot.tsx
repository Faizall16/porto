  ...
  return (
    <>
      ...
      <Button
        ...
        className={cn(
          'fixed h-12 w-12 rounded-full shadow-lg bg-primary-cyan hover:bg-primary-cyan-dark z-50 right-7 bottom-24 transform transition-transform',
          goToTopVisible ? 'translate-y-0' : 'translate-y-12',
          // Pulsing animation when closed
          !isOpen &&
            'after:absolute after:inset-0 after:rounded-full after:border-2 after:border-primary-cyan/50 after:animate-ping'
        )}
        size="icon"
      >
        {isOpen ? (
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <Card
              className={cn(
                'border-primary-cyan/20',
                isMobile && 'h-full rounded-b-none'
              )}
            >
              {/* Header */}
              <div className="p-4 border-b flex items-center justify-between bg-primary-cyan/5">
                <div className="flex items-center gap-2">
                  <Bot className="h-5 w-5 text-primary-cyan" />
                ...
                {isMobile && (
                  <Button
                    ...
                    className="h-8 w-8 rounded-full hover:bg-primary-cyan/10"
                  >
                    <X className="h-4 w-4" />
                    ...
                    messages.map((message) => (
                      <div
                        ...
                        className={cn(
                          ...
                          message.role === 'user'
                            ? 'ml-auto bg-primary-cyan text-white'
                            : 'bg-muted'
                        )}
                      >
              ...
              <form onSubmit={handleSubmit} className="p-4 border-t flex gap-2">
                ...
                <Button
                  ...
                  className="bg-primary-cyan hover:bg-primary-cyan-dark"
                >
                  {isLoading ? (
